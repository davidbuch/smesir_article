...
event_quantile1 <- rowSums(apply(sfor[[names(endpoint[1])]]$event_samples,c(1,3),function(x) quantile(x, c(0.85))))
event_quantile3 <- rowSums(apply(sfor[[names(endpoint[2])]]$event_samples,c(1,3),function(x) quantile(x, c(0.85))))

pdf("output/us_analysis/endpoints/usa_endpoint_forecasts.pdf")
...
abline(v = weeks[endpoint[1]], col="blue")
abline(v = weeks[endpoint[2]], col = "green")
plot_confint(cbind(rowSums(sfor[[names(endpoint[1])]]$expected_confints[(endpoint[1] + 1):endpoint[3],1,]),rowSums(sfor[[names(endpoint[1])]]$expected_confints[(endpoint[1] + 1):endpoint[3],3,])),x=weeks[(endpoint[1] + 1):endpoint[3]],density=30,angle=90,col="blue")
plot_confint(cbind(rowSums(sfor[[names(endpoint[2])]]$expected_confints[(endpoint[2] + 1):endpoint[3],1,]),rowSums(sfor[[names(endpoint[2])]]$expected_confints[(endpoint[2] + 1):endpoint[3],3,])),x=weeks[(endpoint[2] + 1):endpoint[3]],density=30,angle=0,col="green")
lines(x=weeks[(endpoint[1] + 1):endpoint[3]],event_quantile1[(endpoint[1] + 1):endpoint[3]],lty = 3,lwd = 3, col="blue")
lines(x=weeks[(endpoint[2] + 1):endpoint[3]],event_quantile3[(endpoint[2] + 1):endpoint[3]],lty = 4,lwd = 3,col="green")
legend("topleft",legend = c("Forecast 95% CI after 74 weeks","Forecast 95% CI after 77 weeks","Forecast 0.85 Quantile after 74 weeks","Forecast 0.85 Quantile after 77 weeks"), fill = c("blue", "green", NA, NA), density = c(30,30,0,0), angle = c(0,90,NA,NA), border = c("blue", "green", NA, NA), lty = c(NA,NA,3,4), lwd = c(NA, NA, 3,3), col = c(NA,NA, "blue", "green"),cex = 1, bty = "o", bg = "white" )
dev.off()
...
pdf("output/us_analysis/endpoints/cal_endpoint_forecasts.pdf")
...
abline(v = weeks[endpoint[1]], col="blue")
abline(v = weeks[endpoint[2]], col = "green")
plot_confint(sfor[[names(endpoint[1])]]$expected_confints[(endpoint[1] + 1):endpoint[3],c(1,3),k],x=weeks[(endpoint[1] + 1):endpoint[3]],density=30,angle=90,col="blue")
plot_confint(sfor[[names(endpoint[2])]]$expected_confints[(endpoint[2] + 1):endpoint[3],c(1,3),k],x=weeks[(endpoint[2] + 1):endpoint[3]],density=30,angle=0,col="green")
lines(x=weeks[(endpoint[1] + 1):endpoint[3]],cal_event_quantile1[(endpoint[1] + 1):endpoint[3]],lty = 3,lwd = 3, col="blue")
lines(x=weeks[(endpoint[2] + 1):endpoint[3]],cal_event_quantile3[(endpoint[2] + 1):endpoint[3]],lty = 4,lwd = 3,col="green")
legend("topleft",legend = c("Forecast 95% CI after 74 weeks","Forecast 95% CI after 77 weeks","Forecast 0.85 Quantile after 74 weeks","Forecast 0.85 Quantile after 77 weeks"), fill = c("blue", "green", NA, NA), density = c(30,30,0,0), angle = c(0,90,NA,NA), border = c("blue", "green", NA, NA), lty = c(NA,NA,3,4), lwd = c(NA, NA, 3,3), col = c(NA,NA, "blue", "green"),cex = 1, bty = "o", bg = "white" )
dev.off()
...
pdf("output/us_analysis/endpoints/delta2_current_posterior.pdf")
delta_effect_prior <- rnorm(10000, mean = 0, sd = sqrt(sfit[[names(endpoint[2])]]$prior$V0[2]))
hist(delta_effect_prior, col = rgb(0,0,1,0.5), breaks = seq(-20,20,0.2), freq = F, main = "Delta Prevalence Effect", ylim = c(0,3), xlim = c(-5,5), xlab = "Current Posterior")
hist(sfit[[names(endpoint[2])]]$samples$Xi0[,2], col = rgb(0,1,0,0.5), breaks = seq(-5,5,0.2), freq = F, add = T)
hist(sfit[[names(endpoint[3])]]$samples$Xi0[,2], col = rgb(1,0,0,0.5), breaks = seq(-5,5,0.2), freq = F, add = T)
legend("topleft", c("Prior - N(0,10)", "End of week 77", "End of week 90"), fill = c(rgb(0,0,1,0.5),rgb(0,1,0,0.5),rgb(1,0,0,0.5)), cex = 1)
dev.off()

# Delta Prevalence plots
usa_delta_prevalence <- delta_prevalence %*% state_populations/sum(state_populations)
pdf("output/us_analysis/endpoints/usa_delta_prevalence.pdf")
...
abline(v = weeks[endpoint[1]], col="blue")
abline(v = weeks[endpoint[2]], col = "green")
abline(v = weeks[endpoint[3]], col = "red")
...
pdf("output/us_analysis/endpoints/cal_delta_prevalence.pdf")
